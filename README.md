...
- **列规则按页面记忆**：拆分/筛选/格式设置仅存本机，重进同页面自动恢复；面板「恢复默认」可一键重置并清除本页记忆
...
5. 工具栏修改文件名、选择输出方式（默认 xlsx），点「导出」或按 `Enter`；不想落盘可选「复制为表格 (TSV)」贴进 Excel/飞书 或「复制为 Markdown」贴进文档，点按钮即进剪贴板
