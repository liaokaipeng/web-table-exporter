...
- **虚拟滚动全量采集**：vxe-table、el-table-v2、AG Grid 等只渲染可见行的表格，点击后自动滚动采集全部数据并去重，无需手动翻页；窗口重叠按行元素身份判定，相邻内容完全相同的行也不漏采
...
5. 工具栏修改文件名、用导出按钮右侧「▾」下拉选择输出方式（默认 Excel，选过一次即记住），点「导出」或按 `Enter`（多表导出期间可点「停止导出」中止，已下载的文件保留）；不想落盘可选「复制为表格 (TSV)」贴进 Excel/飞书 或「复制为 Markdown」贴进文档，点主按钮即进剪贴板。文件名可写 `{title}` / `{date}` / `{time}` 占位符（鼠标悬停有说明），编辑一次即记住，清空则回到默认命名
