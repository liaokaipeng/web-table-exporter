...
| main.js | 其余全部 | 主 UI / 事件 / 选中管理 / 导出 / 退出清理 / 启动装配；v2.0 新增：toast 反馈系统（结果性通知迁出 hint，成功/信息 2.5s 自动消失、错误常驻可关、同屏 3 条上限）、虚拟采集可中止（「停止采集」genToken 作废当前任务）、导出后保留选择（toast「退出」动作）、设计 token + 深色模式（prefers-color-scheme）+ 动效（prefers-reduced-motion）；v2.1：`hitRoot()` 与表格存在性检查支持 div 网格表格；v2.2：经 `gridRootOf()` / `GRID_ROOT_SELECTOR` 适配器分发，覆盖全部注册组件 |

关键机制：

- **UI/事件**（main.js）：单个 Shadow DOM host（`all:initial` 隔离页面样式），含悬浮高亮层、已选覆盖层、底部工具栏、toast 容器。`mouseover` 委托找 table，经 `hitRoot()` 解析为逻辑表格根（分体结构取包装容器；v2.1 无 table 命中时回落网格表格根，v2.2 经 `gridRootOf()` 适配器分发——单元格内嵌传统 table 时优先命中内层 table，可独立选中）；`click` 捕获阶段拦截页面跳转（工具栏自身放行）；`keydown` Esc 退出 / Enter 导出（拆分面板打开时改为关面板 / 保存）；`scroll`/`resize` rAF 节流重定位。v2.0：设计 token（颜色/圆角 CSS 变量定义于 :host，工具栏与面板共享，深色模式经 `prefers-color-scheme` 覆写）；toast 系统（`toast()` 返回句柄支持进度型复用同一条）；反馈双通道——hint 行只留引导/进行时文案（默认提示、采集进度、导出中），结果性通知全走 toast；采集期间「取消」按钮变「停止采集」（`stopCollect()` 仅 genToken++ 作废任务，不退出选择模式）；选中徽标贴视口边缘时翻内侧（flip-x/y）
