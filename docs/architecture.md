...
| table.js | cell | `getRows()` / `extractTable()` / `makeSheetName()` / `splitGroupOf()` / `pairSplitGroup()`（分体识别 + 纯函数配对，模块级零 DOM 引用，algo-check.cjs 离线回归）；表头行判定 = thead/tfoot 行 + tbody 全 th 行（extractTable 取前导连续段）；v2.1 新增 div 网格表格，v2.2 重构为适配器注册表（el-table-v2 / AG Grid / MUI X DataGrid / Tabulator）：`isGridTable()` 根特征类分发 + `gridRowsOf()` 取行 + `gridScrollEls()` 滚动容器 + `gridRootOf()`/`GRID_ROOT_SELECTOR` 命中入口 + `gridHeaderCellsOf()` 指纹表头分发 + `rowsSortedByRowIndex()` 行序排序纯函数（aria-rowindex，复用行节点组件） |
| virtual.js | table | `isVirtualTable()` / `collectVirtual()` / `overlapLen()`；v2.1：网格表格恒走滚动采集，固定列多分区（left/main/right）联动设置 scrollTop |
...
| main.js | 其余全部 | 主 UI / 事件 / 选中管理 / 导出 / 退出清理 / 启动装配；v2.0 新增：toast 反馈系统（结果性通知迁出 hint，成功/信息 2.5s 自动消失、错误常驻可关、同屏 3 条上限）、虚拟采集可中止（「停止采集」genToken 作废当前任务）、导出后保留选择（toast「退出」动作）、设计 token + 深色模式（prefers-color-scheme）+ 动效（prefers-reduced-motion）；v2.1：`hitRoot()` 与表格存在性检查支持 div 网格表格；v2.2：经 `gridRootOf()` / `GRID_ROOT_SELECTOR` 适配器分发，覆盖全部注册组件；v2.4：选择模式点击放行（非表格点击不拦截 + `pruneDetached()` 剔除被页面交互替换的选中表格，链接拦截改 toast 提示） |

关键机制：

- **UI/事件**（main.js）：单个 Shadow DOM host（`all:initial` 隔离页面样式），含悬浮高亮层、已选覆盖层、底部工具栏、toast 容器。`mouseover` 委托找 table，经 `hitRoot()` 解析为逻辑表格根（分体结构取包装容器；v2.1 无 table 命中时回落网格表格根，v2.2 经 `gridRootOf()` 适配器分发——单元格内嵌传统 table 时优先命中内层 table，可独立选中）；`click` 捕获阶段 v2.4 起点击放行——非表格点击不拦截（翻页/筛选等页面交互可用，`pruneDetached()` 同步剔除被替换的选中表格并 toast 告知，`exporting` 期间跳过），仅表格点选与链接 `a[href]` 导航拦截（链接就地红框高亮 1s + toast 提示，防误跳转丢会话）；采集中仍全拦截（点击给 2s 节流 toast 提示）；`keydown` Esc 退出 / Enter 导出（焦点为页面元素时放行，工具栏按钮走默认行为；拆分面板打开时改为关面板 / 保存）；`scroll`/`resize` rAF 节流重定位。v2.0：设计 token（颜色/圆角 CSS 变量定义于 :host，工具栏与面板共享，深色模式经 `prefers-color-scheme` 覆写）；toast 系统（`toast()` 返回句柄支持进度型复用同一条）；反馈双通道——hint 行只留引导/进行时文案（默认提示、采集进度、导出中），结果性通知全走 toast；采集期间「取消」按钮变「停止采集」（`stopCollect()` 仅 genToken++ 作废任务，不退出选择模式）；选中徽标贴视口边缘时翻内侧（flip-x/y）。v2.4：toast 视觉强化（语义色圆形图标徽标 + 浅色底色 + 文案加粗，warn 警示型停留 4s）；`doExport()` 迭代前快照表格列表（逐表 `yieldToMain()` 让出主线程期间 prune 不影响导出范围与列设置）
...
- **div 网格表格**（table.js，v2.1 引入、v2.2 适配器注册表）：组件库虚拟化表格无 `<table>` 元素——div + ARIA role 模拟结构，只渲染可见窗口行。每组件一个适配器（`name` + `rootSel` + 五钩子 `isRoot/headerRowsOf/bodyRowsOf/scrollElsOf/headerCellsOf`），注册表 `GRID_ADAPTERS` 按序分发，新增组件只加适配器——virtual/main/persist 均经本模块分发入口间接调用，自身零改动。识别 = 组件根特征类（`.el-table-v2__root` / `.ag-root-wrapper` / `.MuiDataGrid-root` / `.tabulator`，后者另校验 `.tabulator-tableHolder` 结构防同名类误报），不依赖结构探测（类名稳定、误报无害）。取行 `gridRowsOf()` = `headerRowsOf()` 表头行 + `bodyRowsOf()` 数据行：多分区组件（el-table-v2 left/main/right、AG Grid pinned/center）表头行按行号对齐拼接、数据行按视觉序 zip 拼接（各分区渲染同一数据的行区间一致）；行节点复用组件（AG Grid / MUI X）DOM 顺序非视觉顺序，先经 `rowsSortedByRowIndex()` 按 `aria-rowindex` 数值升序修正（全部行带该属性才排序，部分缺失整体保持 DOM 序防半排序）。滚动容器 `gridScrollEls()`：el-table-v2 多分区各返回一个（联动设置 scrollTop），其余组件单滚动容器。不参与分体配对（无 table 结构）
- **提取**（table.js `getRows()`）：兼容 thead 直接嵌 th（无 tr）；过滤 virtual-spacer 占位行、`display:none` 隐藏行；网格表格（v2.2 任一注册适配器命中）走 `gridRowsOf()`，普通 table / 分体结构走原路径
...
| 网格表格识别用组件根特征类（v2.1 `isGridTable`） | el-table-v2 无 table 元素无法复用 table 系判定，结构探测（role/绝对定位行）脆弱；`div.el-table-v2__root` 是组件根的稳定特征类，非组件根不会携带；取行按分区 DOM 序 zip 拼接（各分区渲染同一数据的行区间一致），固定列多分区联动滚动防渲染窗口错位 |
| 网格组件适配器注册表（v2.2 `GRID_ADAPTERS`） | 四组件（el-table-v2 / AG Grid / MUI X / Tabulator）的识别与取行差异全部收敛进 table.js 适配器（`name` + `rootSel` + 五钩子），virtual/main/persist 经分发入口间接调用零改动，新增组件只加适配器；rootSel 须互不相同（按注册序分发，重叠会互相遮蔽，algo-check 有断言）；Tabulator 根类 `.tabulator` 过于通用，另校验 `.tabulator-tableHolder` 结构防同名类误报；行节点复用组件（AG Grid / MUI，窗口滚动时 aria-rowindex 重分配、DOM 序非视觉序）经 `rowsSortedByRowIndex()` 数值升序修正（全部行带该属性才排序，部分缺失保持 DOM 序防半排序）；Ant Design Vue Table 原生 table + ant-table 包装类，既有分体配对直接命中，无需适配器 |
