...
| table.js | cell | `getRows()` / `extractTable()` / `makeSheetName()` / `splitGroupOf()` / `pairSplitGroup()`（分体识别 + 纯函数配对，模块级零 DOM 引用，algo-check.cjs 离线回归）；表头行判定 = thead/tfoot 行 + tbody 全 th 行（extractTable 取前导连续段）；v2.1 新增 div 网格表格（el-table-v2）：`isGridTable()` 根特征类判定 + `gridRowsOf()` 分区拼接取行 + `gridScrollEls()` 滚动 window 定位 |
...
| main.js | 其余全部 | 主 UI / 事件 / 选中管理 / 导出 / 退出清理 / 启动装配；v2.0 新增：toast 反馈系统（结果性通知迁出 hint，成功/信息 2.5s 自动消失、错误常驻可关、同屏 3 条上限）、虚拟采集可中止（「停止采集」genToken 作废当前任务）、导出后保留选择（toast「退出」动作）、设计 token + 深色模式（prefers-color-scheme）+ 动效（prefers-reduced-motion）；v2.1：`hitRoot()` 与表格存在性检查支持 div 网格表格（el-table-v2） |

关键机制：

- **UI/事件**（main.js）：单个 Shadow DOM host（`all:initial` 隔离页面样式），含悬浮高亮层、已选覆盖层、底部工具栏、toast 容器。`mouseover` 委托找 table，经 `hitRoot()` 解析为逻辑表格根（分体结构取包装容器；v2.1 无 table 命中时回落 `.el-table-v2__root` 网格表格根——单元格内嵌传统 table 时优先命中内层 table，可独立选中）；`click` 捕获阶段拦截页面跳转（工具栏自身放行）；`keydown` Esc 退出 / Enter 导出（拆分面板打开时改为关面板 / 保存）；`scroll`/`resize` rAF 节流重定位。v2.0：设计 token（颜色/圆角 CSS 变量定义于 :host，工具栏与面板共享，深色模式经 `prefers-color-scheme` 覆写）；toast 系统（`toast()` 返回句柄支持进度型复用同一条）；反馈双通道——hint 行只留引导/进行时文案（默认提示、采集进度、导出中），结果性通知全走 toast；采集期间「取消」按钮变「停止采集」（`stopCollect()` 仅 genToken++ 作废任务，不退出选择模式）；选中徽标贴视口边缘时翻内侧（flip-x/y）
...
- **div 网格表格**（table.js，v2.1）：Element Plus el-table-v2（虚拟化表格）无 `<table>` 元素——div + ARIA role 模拟结构，只渲染可见窗口行。识别 = 组件根特征类 `div.el-table-v2__root`（同元素带 `.el-table-v2`），不依赖结构探测（类名稳定、误报无害——非组件根不会有此类）。取行 `gridRowsOf()`：分区（`gridPartsOf()` 按视觉列序 left → main → right）内 `.el-table-v2__dynamic-header-row` 表头格按行号对齐拼接（多行表头逐行合并）+ `.el-table-v2__body .el-table-v2__row` 数据行按 DOM 顺序 zip 拼接（固定列时各分区渲染同一数据的行区间一致）；占位格（`--placeholder` 类，无宽度列）排除。滚动 window `gridScrollEls()`：各分区 `.el-table-v2__body`（el-vl__wrapper）内首个子 div（overflow:hidden 但编程式 scrollTop 有效，组件监听 scroll 重渲染窗口行）。不参与分体配对（无 table 结构）
- **提取**（table.js `getRows()`）：兼容 thead 直接嵌 th（无 tr）；过滤 virtual-spacer 占位行、`display:none` 隐藏行；v2.1 分发：网格表格走 `gridRowsOf()`，普通 table / 分体结构走原路径
