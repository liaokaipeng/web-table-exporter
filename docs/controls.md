...
按优先级逐层下探，命中的层决定取值来源；均未命中走纯文本兜底：
...
实现上用统一选择器 `CONTROL_SEL` 取「控件候选」（原生表单 + ARIA 角色 + 类名含 switch），
逐个送 `controlValue()` 精确判定：返回替换文本则克隆替换，返回 `null` 则保留原样由 innerText 兜底（误匹配无损）。
...
| `select` 多选 | 各选中项「文本(value)」用顿号「、」隔开 | ✅ |
| `checkbox` / `radio` | 勾选 → 是 / 否 | ✅ |
| `input[type=hidden]` | 忽略（替换为空，不导出） | ✅ |
...
| `[role=slider]` / `[role=spinbutton]` | `aria-valuenow` | ✅ |
