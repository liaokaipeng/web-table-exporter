...
现状（实施前）：底部工具栏（提示文案 + 已选计数 + 文件名 + 格式下拉 + 三按钮）+ 列设置模态面板（每行 7 控件 + 前 3 行预览）；全部状态反馈挤在工具栏 hint 一行。
目标：修复交互缺陷、建立 toast 反馈系统、重构面板信息结构、收敛视觉设计；算法层（split/format/persist/table 纯函数）零改动。以下 P0–P3 与改动清单为实施依据，落地情况见文末实施后记。
...
随 v2.0 一并交付（发布前反馈，见各条「实施后细化」）：

- 工具栏折行形态：三按钮成组（见 P0-3 细化），hint 截断收缩，消除「上排挤满、下排只剩一个按钮」的失衡换行；E2E 轮次 A 补「三按钮成组同父」断言防回归
- 列设置面板宽度 780px → 900px（上限 95vw），预览单元格列宽上限 180px → 220px——长列名（如拆分新列「标题/产品ID_控件1」）显示更完整，配置区更舒展

v2.0.1：

- 面板再加宽 900px → 1000px；拆分子行左缘主色竖线（含深色模式）；预览区限高 24vh 自身滚动 + 表头 sticky（配置与预览始终同屏）；列名截断悬停 title 显全名；展开子行滚入视野
- 默认不拆分：智能预填不再预勾选任何拆分（多行文本列此前默认勾选「按换行拆分」），所有列打开面板即默认收起；展开后的模式预设与按钮 title 建议保留，已保存的拆分规则恢复不受影响
- E2E 同步：默认不拆分改写 fixture 轮次 D/G 与 virtual 断言（断言默认收起 + 手动展开验证预设）；全量回归 fixture 105 项 + virtual 33 项 PASS
